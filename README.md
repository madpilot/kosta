...
| `OPENAI_API_KEY` | *(empty)* | Required when `AI_BACKEND=openai`. |
| `OPENAI_MODEL` | `gpt-4o` | OpenAI model name. |
| `AI_PREAMBLE` | *(built-in)* | Override the default Costa system prompt. |
| `WEATHER_API_KEY` | *(empty)* | OpenWeatherMap key. Forecasts are skipped if unset. |
| `USER_LOCATION` | *(empty)* | e.g. `Perth, AU`. Used for weather + season-aware advice. |
